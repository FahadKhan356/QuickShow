...
const AddShows = () => {
  ...
  const handleAddShow = async () => {
    if (!selectedMovies) return toast("Please select a movie");
    if (!showPrice || showPrice <= 0) return toast("Please enter a valid show price");
    ...
    try {
      await api('/show/add', {
        method: 'POST',
        body: { movieId: selectedMovies, showsInput, showPrice: Number(showPrice) },
      });
      ...
      <>
        ...
        {Object.keys(dateTimeSelection).length > 0 &&
          <div className='mt-5'>
            ...
            <button onClick={handleAddShow} className='bg-primary px-8 py-2 mt-6 text-white rounded hover:bg-primary/90 cursor-pointer'>
              Add Show
            </button>
          </div>}
